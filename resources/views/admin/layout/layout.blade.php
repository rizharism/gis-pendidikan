...
<body class="bg-slate-50 font-sans antialiased text-slate-900" 
    ...
    <div class="flex h-screen overflow-hidden">
        <!-- Sidebar -->
        <aside class="bg-slate-900 text-slate-300 transition-all duration-300 ease-in-out z-40 fixed inset-y-0 left-0 lg:static lg:inset-0"
                ...
                <div class="flex-1 flex items-center justify-between px-4 sm:px-6">
                    <div class="flex items-center gap-4">
                        ...
                        <button @click="sidebarOpen = !sidebarOpen" 
                            ...
                            <svg class="w-6 h-6" :class="{ 'rotate-180': !sidebarOpen }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="relative" x-data="{ open: false }" @click.outside="open = false" @close.stop="open = false">
                            <button @click="open = ! open" class="flex items-center gap-3 p-1.5 rounded-xl hover:bg-slate-50 transition-all border border-transparent hover:border-slate-200">
                                <div class="text-right hidden sm:block">
                                    <p class="text-sm font-semibold text-slate-800 leading-none">{{ Auth::user()->name }}</p>
                                    <p class="text-[10px] text-slate-500 font-medium uppercase tracking-wider mt-1">Admin</p>
                                </div>
                            ...
                            <div x-show="open" 
                                ...
                                <div class="p-2">
                                    ...
                                    <a href="{{ route('admin.profile') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-indigo-50 hover:text-indigo-700 rounded-xl transition-colors group">
                                    ...
                                    <form method="POST" action="{{ route('logout') }}">
                                        @csrf
                                        <button type="submit" class="flex items-center w-full gap-3 px-4 py-2.5 text-sm text-rose-600 hover:bg-rose-50 rounded-xl transition-colors group">
                                            <svg class="w-4 h-4 text-rose-400 group-hover:text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
