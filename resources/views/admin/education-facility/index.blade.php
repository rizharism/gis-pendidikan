    ...
    <div class="space-y-6" x-data="{
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1">Kelola informasi sekolah dan institusi
                    pendidikan.</p>
            </div>
        ...
        @if (session('success'))
            <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 5000)" x-show="show"
                ...
                <button @click="show = false"
                    class="text-emerald-600 hover:text-emerald-800 transition-colors p-1 rounded-lg hover:bg-emerald-100 shrink-0">
        ...
        @if (session('error'))
            <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 5000)" x-show="show"
                ...
                <button @click="show = false"
                    class="text-rose-600 hover:text-rose-800 transition-colors p-1 rounded-lg hover:bg-rose-100 shrink-0">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12">
                        </path>
                    </svg>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col"
                ...
                <template x-if="detailLoading">
                    <div class="flex-1 flex items-center justify-center py-24">
                        <div class="w-10 h-10 border-4 border-brand-accent border-t-transparent rounded-full animate-spin">
                        </div>
                    </div>
                </template>
                ...
                <template x-if="!detailLoading && detail">
                    <div class="flex flex-col max-h-[90vh] overflow-hidden">
                        ...
                        <div class="px-6 pt-6 pb-4 flex items-start justify-between gap-4 shrink-0">
                            <div class="flex-1 min-w-0">
                                <h3 class="text-lg font-black text-slate-800 dark:text-white leading-tight"
                                    x-text="detail.name"></h3>
                                <div class="flex flex-wrap items-center gap-2 mt-1.5">
                                    <span class="text-xs font-bold uppercase text-slate-500 dark:text-slate-400"
                                        x-text="detail.klas"></span>
                                    <template x-if="detail.accreditation">
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-black bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400"
                                            x-text="'Akreditasi ' + detail.accreditation"></span>
                                    </template>
                                    <template x-if="detail.school_code">
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-mono font-bold bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300"
                        ...
                        <div class="flex-1 overflow-y-auto px-6 pb-6 space-y-5">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <template x-if="detail.principal_name">
                                    <div class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/50 space-y-1">
                                        <p class="text-[10px] font-black uppercase tracking-widest text-slate-400"
                                            x-text="detail.klas === 'universitas' ? 'Rektor' : 'Kepala Sekolah'"></p>
                                        <p class="text-sm font-medium text-slate-700 dark:text-slate-200"
                                            x-text="detail.principal_name"></p>
                                    </div>
                                </template>
                                    ...
                                    <div class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/50 space-y-1">
                                        ...
                                        <p class="text-sm font-medium text-slate-700 dark:text-slate-200">
                                            ...
                                            <template x-if="detail.student_capacity && detail.teacher_count"><span
                                                    class="text-slate-300"> · </span></template>
                                ...
                                <template x-if="detail.phone">
                                    <div class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/50 space-y-1">
                                        <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Telepon
                                        </p>
                                ...
                                <template x-if="detail.email">
                                    <div class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/50 space-y-1">
                                        <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Email
                                        </p>
                                ...
                                <template x-if="detail.website">
                                    <div class="sm:col-span-2 p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/50 space-y-1">
                                        <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Website
                                        </p>
                                        <a :href="detail.website" target="_blank" rel="noopener"
                                            class="text-sm font-medium text-brand-accent hover:underline break-all"
                                ...
                                <template x-if="detail.opening_hours && detail.opening_hours.length > 0">
                                    <div class="sm:col-span-2 p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/50 space-y-2">
                                        <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Jam
                                            Operasional</p>
                                        <div class="space-y-1">
                            ...
                            <template
                                x-if="(detail.gallery && detail.gallery.length > 0) || (detail.video_url && ytEmbed)">
                                <div>
                                    <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">Media
                                    </p>
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
                                        ...
                                        <template x-if="detail.gallery && detail.gallery.length > 0">
                                            <div class="space-y-2">
                                                ...
                                                {{-- Main Image --}}
                                                <div
                                                    class="relative rounded-2xl overflow-hidden bg-slate-100 dark:bg-slate-700 aspect-video">
                                                    <img :src="detail.gallery[carouselIdx]"
                                                    ...
                                                    <span
                                                        class="absolute bottom-1.5 right-2 bg-black/50 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full"
                                                ...
                                                <template x-if="!showVideo">
                                                    <button type="button" @click="showVideo = true"
                                                        ...
                                                        <div
                                                            ...
                                                            <div
                                                                class="w-12 h-12 bg-red-600 rounded-full flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform">
                                                        ...
                                                        <span
                                                            class="absolute bottom-1.5 left-2 bg-black/60 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">Putar
                                                            Video</span>
                                                    </button>
                                                </template>
                                                <template x-if="showVideo">
                                                    <div class="rounded-2xl overflow-hidden aspect-video">
                                                        <iframe :src="ytEmbed" class="w-full h-full"
                                                            frameborder="0"
                        ...
                        {{-- Footer --}}
                        <div
                            class="px-6 py-4 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center shrink-0">
