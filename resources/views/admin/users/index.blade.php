...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Manajemen Pengguna</h2>
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1">Kelola akun dan hak akses pengguna sistem.</p>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-50 dark:border-slate-700">
                <form method="GET" action="{{ route('admin.users.index') }}" class="flex gap-2">
                    ...
                    @if($search)
                        <a href="{{ route('admin.users.index') }}" class="bg-slate-200 dark:bg-slate-600 hover:bg-slate-300 dark:hover:bg-slate-500 text-slate-700 dark:text-slate-200 font-bold py-2.5 px-4 rounded-xl transition-all flex items-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                            </svg>
                            Reset
                            ...
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="font-semibold text-slate-700 dark:text-slate-200 text-sm">{{ $user->name }}</span>
                                        @if($user->id === auth()->id())
                                            <span class="text-[10px] bg-blue-100 dark:bg-brand-dark/30 text-brand-accent font-bold px-2 py-0.5 rounded-full">Anda</span>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @else
                                        <span class="inline-flex items-center gap-1 text-xs font-bold bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-3 py-1 rounded-full">
            ...
            <div class="px-6 py-4 border-t border-slate-50 dark:border-slate-700 bg-slate-50/30 dark:bg-slate-700/30">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <p class="text-xs text-slate-500 dark:text-slate-400 font-medium italic">
