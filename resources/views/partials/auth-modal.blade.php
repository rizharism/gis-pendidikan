...
<div x-data="authModal()" @open-auth-modal.window="openModal()" @keydown.escape.window="closeModal()">
    ...
    <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0"
        ...
        {{-- Card --}}
        <div class="relative z-10 w-full max-w-sm bg-white rounded-3xl shadow-2xl overflow-hidden" x-trap="open">
            ...
            <div x-show="toast" x-cloak x-transition
                ...
                :class="toastIsError ? 'bg-rose-50 text-rose-700 border border-rose-100' :
            ...
            <template x-if="mode === 'login'">
                <div class="p-6 sm:p-8">
                    ...
                    <div class="text-center mb-6">
                        ...
                        <h2 class="text-xl font-bold text-slate-800">Masuk Panel Admin</h2>
                    ...
                    <form @submit.prevent="login()" class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input id="auth-password" :type="showPassword ? 'text' : 'password'" x-model="password"
                                    required
                        ...
                        <div class="flex items-center justify-between">
                            <label class="inline-flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" x-model="remember"
                                    class="rounded border-slate-300 text-brand-dark focus:ring-brand-accent">
            ...
            <template x-if="mode === 'forgot'">
                <div class="p-6 sm:p-8">
                    <div class="text-center mb-6">
                        ...
                        <p class="text-sm text-slate-400 mt-1">Masukkan email Anda untuk menerima kode verifikasi 6
                            digit.</p>
                    </div>
            ...
            <template x-if="mode === 'reset'">
                <div class="p-6 sm:p-8">
                    <div class="text-center mb-6">
                        ...
                        <p class="text-sm text-slate-400 mt-1">Masukkan kode 6 digit yang dikirim ke <strong
                                x-text="resetEmail"></strong>.</p>
                    </div>
                    ...
                    <form @submit.prevent="resetPassword()" class="space-y-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Kode
                                Verifikasi</label>
                        ...
                        <div>
                            <label
                                class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Password
                                Baru</label>
                            <div class="relative">
                                <input :type="showNewPassword ? 'text' : 'password'" x-model="newPassword" required
                                    minlength="8"
                                    class="w-full rounded-xl border border-slate-200 px-4 py-2.5 pr-10 text-sm text-slate-800 focus:border-brand-accent focus:ring-brand-accent transition"
                                    placeholder="Min. 8 karakter">
                                <button type="button" @click="showNewPassword = !showNewPassword"
                                    class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
                                    <svg x-show="!showNewPassword" class="w-4 h-4" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <svg x-show="showNewPassword" x-cloak class="w-4 h-4" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
                                    </svg>
                        ...
                        <div>
                            <label
                                class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Konfirmasi
                                Password</label>
